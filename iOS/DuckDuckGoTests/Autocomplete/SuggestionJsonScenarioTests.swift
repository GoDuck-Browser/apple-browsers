...
final class SuggestionJsonScenarioTests: XCTestCase {
    ...
    private func runJsonTestScenario(_ testScenario: TestScenario, named name: String) async throws {
        ...
        featureFlagger.enabledFeatureFlags.append(.autcompleteTabs)
        ...
        let suggestionLoader = SuggestionLoader()
        ...
        // Request suggestions
        suggestionLoader.getSuggestions(query: input.query, usingDataSource: dataSource) { results, error in
...
extension SuggestionJsonScenarioTests {
    ...
    struct TestInput: Decodable {
        ...
        func isURLIgnored(_ url: URL) -> Bool {
            ignoredUris?.contains(url.absoluteString) ?? false
        }
    }
...
private extension Suggestion {
    ...
    func expectedSuggestion(query: String) -> SuggestionJsonScenarioTests.TestExpectations.ExpectedSuggestion? {
        ...
        case .phrase(phrase: let phrase):
            return .init(type: .phrase, title: phrase, subtitle: subtitle, uri: URL.makeSearchURL(query: phrase)?.absoluteString, tabId: nil, score: 0)
